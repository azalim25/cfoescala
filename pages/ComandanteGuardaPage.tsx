                        ...
                        <table className="w-full text-left border-collapse">
                            <thead className="sticky top-0 z-10 bg-slate-50 dark:bg-slate-900 shadow-sm">
                                <tr className="bg-slate-50 dark:bg-slate-800/50">
                                    ...
                                    <th className="px-6 py-4 text-[11px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800 text-center bg-slate-100/50 dark:bg-slate-700/50">Total</th>
                                </tr>
                            </thead>
                            <tbody>
                                {sortedMilitaries.length === 0 ? (
                                    ...
                                    sortedMilitaries.map(mil => {
                                        const counts = shiftStats[mil.id] || [0, 0, 0, 0, 0, 0, 0];
                                        ...
                                        return (
                                            <tr key={mil.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 border-b border-slate-50 dark:border-slate-800 text-center bg-slate-50/30 dark:bg-slate-800/20">
                                                    <span className={`inline-flex items-center justify-center px-2 py-1 ${total > 0 ? 'bg-primary text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'} rounded-md text-xs font-bold min-w-[28px]`}>
                                                        {total}
                                                    </span>
                                                </td>
                                            </tr>
